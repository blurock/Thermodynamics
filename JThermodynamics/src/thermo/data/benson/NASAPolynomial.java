...
public class NASAPolynomial implements ThermodynamicInformation {
	...
	/**
	 * The phase of the molecule Here it is, by default gas (G)
	 */
	public String phase;
	/**
	 ...
	 *
	 * @param formula
	 *            The formula (isomer name)
	 * @throws IOException
	 *             For an illegal formula
	 */
	 ...
	 * 
	 * @param nasalines
	 *            The 4 lines of a NASA polynomial
	 * @throws IOException
	 *             If an error occurs in parsing
	 */
	 ...
	 *
	 * The tokenizer should have at least 4 tokens, one for each line of the
	 * NASA polynomial
	 *
	 * @param nasatok
	 *            The tokenizer with 4 elements
	 * @throws IOException
	 *             If an error occurs in parsing
	 */
	public void parse(StringTokenizer nasatok) throws IOException {
		if (nasatok.countTokens() > 4) {
			...
			if (nasatok.countTokens() > 0) {
				error = "not enough lines for NASA polynomial: " + nasatok.countTokens() + "  on line: "
						+ nasatok.nextToken();
			} else {
	 ...
	 *
	 * @throws IOException
	 *             The lines don't contain the right info
	 */
	...
	double computeHeatCapacity(double[] a, double t) {
		double cp = a[0] + t * (a[1] + t * (a[2] + t * (a[3] + t * (a[4]))));
		return cp;
	}

	double computeEnthalpy(double[] a, double t) {
		double enthalpy = a[0] + t * (a[1] / 2.0 + t * (a[2] / 3.0 + t * (a[3] / 4.0 + t * (a[4] / 5.0)))) + a[5] / t;
		return enthalpy;
	}

	double computeEntropy(double[] a, double t) {
		double entropy = a[0] * Math.log(t) + t * (a[1] + t * (a[2] / 2.0 + t * (a[3] / 3.0 + t * (a[4] / 4.0))))
				+ a[6];
		return entropy;
	}
